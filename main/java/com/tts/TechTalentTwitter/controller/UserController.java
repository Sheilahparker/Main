...
public class UserController {
	...
	public String getUsers(@RequestParam(value = "filter", required = false) String filter, Model model) {
		...
		if (filter.equalsIgnoreCase("followers")) {  // getting followers 
			users = usersFollowers;
			model.addAttribute("filter", "followers");
			
		} else if (filter.equalsIgnoreCase("following")) { // getting user's followers
			users = usersFollowing;
			model.addAttribute("filter", "following");
		} else {
			users = userService.findAll();
			model.addAttribute("filter", "all"); // getting all users
		}
